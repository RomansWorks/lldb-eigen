...
class EigenMatrixChildProvider:
    ...
    def __init__(self, valobj, internal_dict):
        self._valobj = valobj
        # print("parsing type: ",  valobj.GetType(), " canonical name: ",  valobj.GetType().GetCanonicalType().GetName()) #, " scalar type: ",  valobj.GetType().GetCanonicalType().GetTemplateArgumentType(0))
        ...
        template_begin = name.find("Eigen::Matrix<")
        template_end = name.find(">", template_begin)
        template_args = name[(template_begin + 1):template_end].split(",")

        if len(template_args) >= 5:
            # Sufficient information was extracted from the template
            ...
            self._unknown_obj = False
        elif len(template_args) == 1:
            # Assume not enough information about compile time, but we can still work with runtime
            self._rows_compile_time = -1
            self._cols_compile_time = -1
            self._fixed_storage = False
            self._row_major = False
            self._unknown_obj = False
        else:
            print("eigenlldb failed to parse matrix definition: ", name, template_args)
            self._unknown_obj = True
